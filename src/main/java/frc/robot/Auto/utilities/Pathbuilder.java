...
public class Pathbuilder extends Command {
    ...
    static PathPlannerPath toReefPath;
    static PathPlannerPath toSourcePath;
    ...
    public void execute() {
        if ((oldSourcePose != RobotContainer.getSourcePose()) || (RobotContainer.getReefPose() != oldReefPose)) {
            oldReefPose = RobotContainer.getReefPose();
            oldSourcePose = RobotContainer.getSourcePose();
            toReefWaypoints = PathPlannerPath.waypointsFromPoses(PoseUtil.offsetPose(oldReefPose, 0, -1), oldReefPose);
            toSourceWaypoints =
                    PathPlannerPath.waypointsFromPoses(PoseUtil.offsetPose(oldSourcePose, 0, -1), oldSourcePose);

            toReefPath = new PathPlannerPath(
                    toReefWaypoints,
                    DriveConstants.CONSTRAINTS,
                    new IdealStartingState(0.5, RobotContainer.getReefPose().getRotation()),
                    new GoalEndState(0, RobotContainer.getReefPose().getRotation()));
            toSourcePath = new PathPlannerPath(
                    toSourceWaypoints,
                    DriveConstants.CONSTRAINTS,
                    new IdealStartingState(0.5, RobotContainer.getSourcePose().getRotation()),
                    new GoalEndState(0, RobotContainer.getSourcePose().getRotation()));
        }
    }
